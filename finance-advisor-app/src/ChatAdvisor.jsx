...
const ChatAdvisor = () => {
  ...
  return (
    <div className="max-w-xl mx-auto my-8 p-5 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-[#181a1b] shadow-lg">
      <h2 className="text-2xl font-bold text-center mb-4 text-gray-900 dark:text-white">Chat with Your Financial Advisor</h2>
      <button onClick={handleClearChat} className="block mx-auto mb-4 bg-red-500 hover:bg-red-600 text-white rounded px-5 py-2 font-semibold transition">Clear Chat</button>
      <div className="min-h-[200px] max-h-[300px] overflow-y-auto bg-gray-100 dark:bg-[#23272a] p-3 rounded mb-4">
        {hasMore && (
        ...
        {chatHistory.map((msg, idx) => {
          ...
          return (
            <div key={msg.id || idx} className={`flex ${isUser ? 'flex-row-reverse' : 'flex-row'} items-end mb-3`}>
              ...
              <div className={
                isUser
                  ? 'bg-blue-50 text-blue-800 dark:bg-blue-900 dark:text-blue-100 rounded-lg px-4 py-2 max-w-[70%] shadow'
                  : 'bg-green-50 border-l-4 border-green-400 text-green-900 dark:bg-green-900 dark:text-green-100 p-4 rounded shadow-md max-w-[70%]'
              }>
        ...
        {loading && (
          <div className="flex flex-row items-center mb-2">
            ...
            <div className="bg-green-50 border-l-4 border-green-400 text-green-900 dark:bg-green-900 dark:text-green-100 p-4 rounded shadow-md max-w-[70%] italic">
              Advisor is typing...
            </div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <label className="text-gray-700 dark:text-gray-200 font-medium">
          ...
          <textarea
            ...
            rows={3}
            className="w-full rounded-lg border border-gray-300 dark:border-gray-600 p-2 text-[15px] bg-white text-gray-900 dark:bg-[#23272a] dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-400"
            disabled={loading}
            required
            placeholder="Type your question and press Enter..."
          />
        </label>
        <button type="submit" disabled={loading || !question.trim()} className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-3 font-semibold text-lg transition disabled:opacity-60 disabled:cursor-not-allowed">{loading ? 'Asking...' : 'Ask Advisor'}</button>
      </form>
      {/* Error Toast */}
      {showToast && error && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 font-semibold">
          {error}
        </div>
      )}
    </div>
